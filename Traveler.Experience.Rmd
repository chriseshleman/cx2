...
I need to join the labels to the main data set as the new variable names. 
I'd been doing that by slapping them together, but a formal merge might be in order. 
...
OK, judging by the first 20 and last 20 variable names for both data sets, they look identical. 
...
So now I've got things lined up. But the main data set is horizontal, and I want to try and merge horizontally using the variable names. This is where it got tricky last time.
...
jdnames.t = as.data.frame(t(jdnames)) 
...
A check of the data confirms that the first cluster of variables are all missing around 21 or 22 percent of their data. 
...
Do a couple of things in order: 
...
```{r, include=TRUE, echo=FALSE}
subset(share.Na, share.Na>0 & share.Na<15) #21
```

.
Then 140 variables missing exactly 21.636 percent of their observations! Here they are: 
```{r, include=TRUE, echo=FALSE}
subset(share.Na, share.Na>21.63601 & share.Na<21.6361) 
```
...
##Future steps? 
Visualizing the number of missing observations in one variable faceted (grouped) by another variable may shed light on whether they are missing by survey month - perhaps one question was asked during the April survey but not the other three surveys. Or maybe it varied by the person conducting the survey. Those are the two potential answers that come to mind. 
